...
export const DynamicTemplateThumbnail = ({ objects, name, className }) => {
    const frame = objects.find(obj => obj.type === 'frame');
    if (!frame) {
        return (<div className={`flex items-center justify-center bg-neutral-100 text-neutral-400 ${className}`}>
    ...
    return (<div className={`relative overflow-hidden pointer-events-none border border-neutral-100 ${className}`} style={{ backgroundColor: frame.fill }}>
      {containedObjects.map(obj => {
            ...
            {obj.type === 'text' && (<div className="w-full text-center truncate leading-none font-bold" style={{ fontSize: 'inherit' }}>
                {obj.text}
              </div>)}
            {obj.type === 'icon' && obj.iconName && (<IconRenderer name={obj.iconName} size="100%" color={obj.fill} strokeWidth={obj.strokeWidth || 2}/>)}
          </div>);
        })}
    </div>);
};
