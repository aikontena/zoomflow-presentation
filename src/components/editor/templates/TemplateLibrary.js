...
function TemplateCard({ template, viewMode, isSelected, isFavorite, onClick, onFavorite }) {
    return (<div onClick={onClick} className={`group cursor-pointer transition-all ${viewMode === 'grid'
            ? 'flex flex-col bg-white border border-neutral-200 rounded-xl overflow-hidden hover:shadow-xl hover:-translate-y-1'
            : 'flex items-center gap-4 p-3 bg-white border border-neutral-100 rounded-lg hover:border-primary'} ${isSelected ? 'ring-2 ring-primary ring-offset-2' : ''}`}>
      <div className={`relative bg-neutral-100 overflow-hidden ${viewMode === 'grid' ? 'aspect-video' : 'w-32 aspect-video rounded-md shrink-0'}`}>
        <DynamicTemplateThumbnail objects={template.objects} name={template.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
          <Button variant="secondary" size="sm" className="opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 transition-all shadow-lg">
            Quick View
          </Button>
        </div>
        <button onClick={onFavorite} className={`absolute top-2 right-2 p-1.5 rounded-full backdrop-blur-md transition-all ${isFavorite
      ...
      <div className="flex-1 p-4 flex flex-col gap-1 min-w-0">
        ...
        <div className="mt-auto pt-3 flex items-center gap-4 text-[11px] text-neutral-400">
          ...
          {viewMode === 'grid' && (<div className="flex items-center gap-1 ml-auto">
              <TrendingUp size={12}/>
              {template.popularity}
            </div>)}
        </div>
...
export default function TemplateLibrary() {
    ...
    return (<div className="flex h-full bg-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-neutral-200">
        ...
        <div className="p-4 border-b border-neutral-100 bg-neutral-50/30 space-y-4">
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search templates..." className="pl-10 bg-white" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
            </div>
            <DropdownMenu>
          ...
          <ScrollArea className="w-full whitespace-nowrap pb-2">
            <div className="flex gap-2">
              {categories.map(cat => (<Button key={cat} variant={activeCategory === cat ? 'default' : 'outline'} size="sm" className="rounded-full px-4" onClick={() => setActiveCategory(cat)}>
                  {cat}
                </Button>))}
            </div>
          </ScrollArea>
        ...
        <ScrollArea className="flex-1">
          <div className="p-6 space-y-8">
            {/* Favorites Section */}
            {favorites.length > 0 && searchQuery === '' && activeCategory === 'All' && (<section className="space-y-4">
                <div className="flex items-center gap-2 text-neutral-900">
                  <Star size={18} className="text-yellow-400 fill-current"/>
                  <h2 className="text-lg font-bold">Your Favorites</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {TEMPLATES.filter(t => favorites.includes(t.id)).map(template => (<TemplateCard key={template.id} template={template} viewMode={viewMode} isSelected={selectedTemplate?.id === template.id} isFavorite={true} onClick={() => setSelectedTemplate(template)} onFavorite={(e) => toggleFavorite(e, template.id)}/>))}
                </div>
                <Separator />
              </section>)}
            ...
            <section className="space-y-4">
              ...
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'flex flex-col gap-3'}>
                {filteredTemplates.map(template => (<TemplateCard key={template.id} template={template} viewMode={viewMode} isSelected={selectedTemplate?.id === template.id} isFavorite={favorites.includes(template.id)} onClick={() => setSelectedTemplate(template)} onFavorite={(e) => toggleFavorite(e, template.id)}/>))}
              </div>
            </section>
          </div>
        </ScrollArea>
      ...
      {selectedTemplate && (<div className="w-80 flex flex-col bg-neutral-50/50 animate-in slide-in-from-right duration-300">
          <ScrollArea className="flex-1">
            <div className="p-6 space-y-6">
              <div className="aspect-video rounded-xl overflow-hidden shadow-lg border border-neutral-200">
                <DynamicTemplateThumbnail objects={selectedTemplate.objects} name={selectedTemplate.name} className="w-full h-full"/>
              </div>
            ...
            ].map((feat, i) => (<div key={i} className="flex items-center gap-3 text-sm text-neutral-700">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {['#3B82F6', '#1E293B', '#F1F5F9', '#FFFFFF'].map((color, i) => (<div key={i} className="w-8 h-8 rounded-full border border-neutral-200 shadow-sm" style={{ backgroundColor: color }}/>))}
                </div>
              </div>
            </div>
          </ScrollArea>
